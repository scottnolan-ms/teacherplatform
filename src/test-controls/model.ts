...
export interface Attempt { id: number; name: string; group: string; mode: Mode; status: Status; previous?: Status; remaining: number; progress: number; start: string; due: string; pausedAt?: number; version: number; expires?: string; closedAt?: string; accuracy?: number; timeSpent?: number }
...
export function applyAction(rows:Attempt[],ids:number[],action:Action,opts:Options,now:number):Attempt[] {
 ...
 if(action==='Reassign'){extra.push({...s,id:nextId++,group:opts.group,status:'Scheduled',progress:opts.fresh?0:s.progress,version:s.version+(opts.fresh?1:0),remaining:opts.minutes*60,start:opts.start,due:opts.due,pausedAt:undefined,closedAt:undefined,expires:new Date(new Date(opts.due).getTime()+86400000).toISOString(),timeSpent:opts.fresh?0:s.timeSpent});return s;}
 ...
 if(action==='Restart')return {...s,status:'In progress' as Status,start:new Date(now).toISOString(),due:opts.due,remaining:opts.minutes*60,progress:opts.fresh?0:s.progress,version:s.version+(opts.fresh?1:0),pausedAt:undefined,closedAt:undefined,expires:new Date(new Date(opts.due).getTime()+86400000).toISOString(),timeSpent:opts.fresh?0:s.timeSpent};
...
export function demoRows(scenario:DemoScenario='Mixed states'):Attempt[] {
 ...
 return seed.map((s,i)=>{
  ...
  return {...s,progress:progress[i],accuracy:accuracy[i],timeSpent:progress[i]?12+i*3:0,
